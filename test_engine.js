...
for(const [d,expected] of [[1,20],[2,400],[3,8902],[4,197281]])assert(perft(parseFen(START_FEN),d)===expected,`perft ${d}`);
const pinned=parseFen('4r1k1/8/8/8/8/8/4R3/4K3 w - - 0 1');assert(!legalMoves(pinned).some(m=>m.from===12&&m.to===13),'king exposure test');
const dead=parseFen('8/8/8/8/8/8/2k5/4K3 w - - 0 1');assert(gameStatus(dead)==='1/2-1/2','dead material test');
const rep=parseFen(START_FEN);rep.history=[positionKey(rep),positionKey(rep),positionKey(rep),positionKey(rep)];assert(gameStatus(rep)==='1/2-1/2','fivefold repetition test');
